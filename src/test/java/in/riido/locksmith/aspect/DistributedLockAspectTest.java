...
class DistributedLockAspectTest {
  ...
  void setUp() {
    ...
    LocksmithProperties lockProperties = new LocksmithProperties(10L, 60L, "lock:");
  ...
  private void setupAnnotation(
    ...
    when(annotation.leaseTimeMinutes()).thenReturn(leaseTimeMinutes);
    when(annotation.waitTimeSeconds()).thenReturn(waitTimeSeconds);
  ...
  class SkipImmediatelyModeTests {
    ...
    void shouldAcquireLockAndExecuteMethod() throws Throwable {
      setupAnnotation(
          "test-lock", LockAcquisitionMode.SKIP_IMMEDIATELY, -1, -1, SkipBehavior.THROW_EXCEPTION);
    ...
    void shouldSkipExecutionWhenLockNotAvailable() throws Throwable {
      setupAnnotation(
          "test-lock", LockAcquisitionMode.SKIP_IMMEDIATELY, -1, -1, SkipBehavior.RETURN_DEFAULT);
      when(redissonClient.getLock("lock:test-lock")).thenReturn(lock);
  ...
  class WaitAndSkipModeTests {
    ...
    void shouldWaitForLockAndExecuteMethod() throws Throwable {
      setupAnnotation(
          "test-lock", LockAcquisitionMode.WAIT_AND_SKIP, -1, -1, SkipBehavior.THROW_EXCEPTION);
      when(redissonClient.getLock("lock:test-lock")).thenReturn(lock);
    ...
    void shouldSkipExecutionAfterWaitTimeout() throws Throwable {
      setupAnnotation(
          "test-lock", LockAcquisitionMode.WAIT_AND_SKIP, -1, -1, SkipBehavior.RETURN_DEFAULT);
      when(redissonClient.getLock("lock:test-lock")).thenReturn(lock);
  ...
  class CustomConfigurationTests {
    ...
    void shouldUseCustomLeaseTimeFromAnnotation() throws Throwable {
      setupAnnotation(
          "test-lock", LockAcquisitionMode.SKIP_IMMEDIATELY, 5, -1, SkipBehavior.THROW_EXCEPTION);
      when(redissonClient.getLock("lock:test-lock")).thenReturn(lock);
    ...
    void shouldUseCustomWaitTimeFromAnnotation() throws Throwable {
      setupAnnotation(
          "test-lock", LockAcquisitionMode.WAIT_AND_SKIP, -1, 30, SkipBehavior.THROW_EXCEPTION);
      when(redissonClient.getLock("lock:test-lock")).thenReturn(lock);
  ...
  class LockKeyTests {
    ...
    void shouldUseKeyPrefixFromProperties() throws Throwable {
      setupAnnotation(
          "my-task", LockAcquisitionMode.SKIP_IMMEDIATELY, -1, -1, SkipBehavior.RETURN_DEFAULT);
    ...
    void shouldThrowIllegalArgumentExceptionWhenKeyIsEmpty() {
      setupAnnotation(
          "", LockAcquisitionMode.SKIP_IMMEDIATELY, -1, -1, SkipBehavior.THROW_EXCEPTION);
  ...
  class ConfigurationValidationTests {
    ...
    @Test
    @DisplayName("Should use default lease time when configured value is zero")
    void shouldUseDefaultLeaseTimeWhenZero() {
      LocksmithProperties props = new LocksmithProperties(0L, 60L, "lock:");

      assertEquals(10L, props.leaseTimeMinutes());
    }

    @Test
    @DisplayName("Should use default wait time when configured value is negative")
    void shouldUseDefaultWaitTimeWhenNegative() {
      LocksmithProperties props = new LocksmithProperties(10L, -30L, "lock:");

      assertEquals(60L, props.waitTimeSeconds());
    }

    @Test
    @DisplayName("Should allow zero wait time")
    void shouldAllowZeroWaitTime() {
      LocksmithProperties props = new LocksmithProperties(10L, 0L, "lock:");

      assertEquals(0L, props.waitTimeSeconds());
    }

    @Test
    @DisplayName("Should use default key prefix when null")
    void shouldUseDefaultKeyPrefixWhenNull() {
      LocksmithProperties props = new LocksmithProperties(10L, 60L, null);
    ...
    void shouldUseDefaultKeyPrefixWhenBlank() {
      LocksmithProperties props = new LocksmithProperties(10L, 60L, "   ");

      assertEquals("lock:", props.keyPrefix());
    }

    @Test
    @DisplayName("Should use custom values when valid")
    void shouldUseCustomValuesWhenValid() {
      LocksmithProperties props = new LocksmithProperties(15L, 90L, "mylock:");

      assertEquals(15L, props.leaseTimeMinutes());
      assertEquals(90L, props.waitTimeSeconds());
